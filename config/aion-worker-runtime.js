  ...
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an AION worker executing a registered task. Analyze the task and return a concise actionable result. Do not claim external side effects were performed unless the task execution adapter actually performed them.'
        },
...
export async function processOne() {
  const task = (await listTasks()).find(item => item.status === 'ready' && item.approvedAt !== undefined || item.status === 'ready' && !item.requiresHumanApproval);
