...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && path === 'ledger') return res.status(200).json({ success: true, ledger: getLedger(url.searchParams.get('account')) });
    if (req.method === 'GET' && path === 'assets') return res.status(200).json({ success: true, assets: listTokenizedAssets() });
    if (req.method === 'GET' && path === 'robots') return res.status(200).json({ success: true, robots: listRobots() });
    ...
    if (path === 'ledger/propose') {
      const proposal = proposeLedgerTransfer(body);
      return res.status(202).json({ success: true, proposal });
    }
    if (path === 'assets/register') {
      return res.status(201).json({ success: true, asset: registerTokenizedAsset(body) });
    }
    if (path === 'robots/register') {
      return res.status(201).json({ success: true, robot: registerRobot(body) });
    }
